...
const init = (_, vm) => {
  ...
  uni.$u.http.interceptors.response.use(
    response => {
      ...
      const successCodes = Array.isArray(RESPONSE_SUCCESS_CODE) ? RESPONSE_SUCCESS_CODE : [RESPONSE_SUCCESS_CODE]

      if (!successCodes.includes(code)) {
        const message = errMsg || msg
        if (message) {
          if (errorMessageMode == 'message') msgError(message)
          if (errorMessageMode == 'modal') alert(message)
        }
    ...
    response => {
      const msg = response.data[RESPONSE_MSG_KEY]
      const errMsg = response.data[RESPONSE_ERR_MSG_KEY]
      const message = checkStatus(response.statusCode, msg || errMsg || '');
      if (message) {
